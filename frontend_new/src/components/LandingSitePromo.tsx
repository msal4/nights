...
const LandingSitePromo: FunctionComponent<LandingSitePromoProps> = ({className, onVisit, onWatch, logo, image}) => {
    ...
    <div className={`landing-site-promo-container-placeholder ${className || ''}`}>
      <div className="landing-site-promo-container relative py-6 border-1 overflow-hidden" >
        ...
        <div className="action-btn-container mt-auto z-10">
          <button className="action-btn px-8 py-3 rounded-full text-sm font-semibold shadow-xl transition-background duration-200 bg-n-red hover:bg-red-800"
            onClick={onWatch}>
            {t('watchNow'}
          </button>
          <button className="action-btn ml-8 px-8 py-3 rounded-full text-sm font-semibold shadow-xl transition-background duration-200 text-black bg-white hover:bg-black hover:text-white"
            onClick={onVisit}
          >
            {t('visitSite')}
          </button>
