...
const Home = () => {
  ...
  const mediaClient = useRemoteMediaClient();
        ...
        <View
          ...
          {isPrivate ? (
            <TouchableOpacity
              ...
              onPress={() => {
                if (promo?.type === 'm') {
                  if (mediaClient && castState === CastState.CONNECTED) {
                    playMovieRemotely(promo, mediaClient);
                    return;
                  }
                  navigation.navigate('WebPlayer', {url: `${privateBase}/movie/${promo.id}/play`});
                  // navigation.navigate('MoviePlayer', { title: promo });
                } else {
                  if (!promo) return;
                  if (mediaClient && castState === CastState.CONNECTED) {
                    playCurrentEpisodeRemotely(promo, mediaClient, token);
                    return;
                  }
                  navigation.navigate('WebPlayer', {url: `${privateBase}/series/${promo.id}/auto/auto/play`});
                        ...
                        {isPrivate ? (
                          <TouchableOpacity
                            ...
                            onPress={() => {
                              if (picked?.type === 'm') {
                                if (mediaClient && castState === CastState.CONNECTED) {
                                  playMovieRemotely(picked, mediaClient);
                                  return;
                                }
                                navigation.navigate('WebPlayer', {
                                  url: `${privateBase}/movie/${picked.id}/play`,
                                });
                                // navigation.navigate('MoviePlayer', { title: promo });
                              } else {
                                if (!picked) {
                                ...
                                if (mediaClient && castState === CastState.CONNECTED) {
                                  playCurrentEpisodeRemotely(picked, mediaClient, token);
                                  return;
                                }
                                navigation.navigate('WebPlayer', {
                                  url: `${privateBase}/series/${picked.id}/auto/auto/play`,
                                });
                                // navigation.navigate('SeriesPlayer', { title: promo });
                              }
                              // if (picked.type === 'm') {
